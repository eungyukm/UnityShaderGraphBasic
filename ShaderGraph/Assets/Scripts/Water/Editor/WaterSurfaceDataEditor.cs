...
{
    [SerializeField] ReorderableList waveList;

    private void OnValidate()
    {
        var init = serializedObject.FindProperty("_init");
        if (init?.boolValue == false) Setup();
        ...
        waveList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
        {
            ...
            // Wavelength
            Rect lengthRect = new Rect(rect.x + ampRect.width, rect.y + standardLine, rect.width * 0.5f,
                standardHeight);
            ...
            Rect omniToggleRect = new Rect(rect.x + rect.width * 0.5f, dirToggleRect.y, rect.width * 0.5f,
                standardHeight);
            ...
            if (!wTypeBool)
            {
                ...
                if (GUI.Button(buttonRect, "Align with Scene Camera")) waveDir.floatValue = CameraRelativeDirection();
            }
            else
            {
                // Omni-Directional
        ...
        waveList.onCanRemoveCallback = (ReorderableList l) => { return l.count > 1; };
        ...
        waveList.onAddCallback = (ReorderableList l) =>
        {
            ...
            {
                EditorUtility.DisplayDialog("Warning!", "You have reached the limit of 10 waves for this Water.",
                    "Close");
            }
        };
        //Draw header
        waveList.drawHeaderCallback = (Rect rect) => { EditorGUI.LabelField(rect, "Wave List"); };
    ...
    public override void OnInspectorGUI()
    {
        ...
        switch (foamType.intValue)
        {
            case 0: //// Auto ////
            {
                EditorGUILayout.HelpBox("Automatic will distribute the foam suitable for an average swell",
                    MessageType.Info);
            }
                break;
            case 1: //// Simple ////
            ...
            case 2: //// Simple ////
            {
                ...
                liteFoam.animationCurveValue = EditorGUILayout.CurveField(liteFoam.animationCurveValue,
                    new Color(0.5f, 0.75f, 1f, 1f), new Rect(Vector2.zero, Vector2.one));
                var mediumFoam = foamSettings.FindPropertyRelative("mediumFoam");
                mediumFoam.animationCurveValue = EditorGUILayout.CurveField(mediumFoam.animationCurveValue,
                    new Color(0f, 0.5f, 1f, 1f), new Rect(Vector2.zero, Vector2.one));
                var denseFoam = foamSettings.FindPropertyRelative("denseFoam");
                denseFoam.animationCurveValue = EditorGUILayout.CurveField(denseFoam.animationCurveValue, Color.blue,
                    new Rect(Vector2.zero, Vector2.one));
                EditorGUILayout.EndHorizontal();
            }
                break;
        ...
        switch (customWaves.boolValue ? 1 : 0)
        {
            case 0: //// Automatic ////
            {
                ...
                EditorGUILayout.Slider(avgWavelength, 1.0f, 200.0f, new GUIContent("Avg Wavelength", avgWavelengthTT),
                    null);
                ...
                randSeed.intValue =
                    EditorGUILayout.IntField(new GUIContent("Random Seed", randSeedTT), randSeed.intValue);
    ...
    void Setup()
    {
        URPWaterSurfaceData wsd = (URPWaterSurfaceData) target;
        wsd._init = true;
    ...
    private string foam3curvesTT =
        "These three curves control the Lite, Medium and Dense foam propagation. X is wave height and Y is foam opacity";

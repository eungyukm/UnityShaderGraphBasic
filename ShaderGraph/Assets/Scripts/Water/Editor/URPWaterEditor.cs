...
[CustomEditor(typeof(URPWater))]
public class URPWaterEditor : Editor
{
    public override void OnInspectorGUI()
    {
        serializedObject.Update();
        URPWater w = (URPWater) target;

        var waterSettingsData = serializedObject.FindProperty("settingsData");
        if (waterSettingsData != null)
        {
            Debug.Log("waterSettingsData is not null!!");
        }
        else
        {
            Debug.Log("waterSettingsData is null!!");
        }
        
        EditorGUILayout.PropertyField(waterSettingsData, true);
        if (waterSettingsData.objectReferenceValue != null)
        {
            CreateEditor((URPWaterSettingsData) waterSettingsData.objectReferenceValue).OnInspectorGUI();
        }
        else
        {
            Debug.Log("waterSettingsData objectReferenceValue is null!");
        }
        ...
        {
            CreateEditor((URPWaterSurfaceData) waterSurfaceData.objectReferenceValue).OnInspectorGUI();
        }

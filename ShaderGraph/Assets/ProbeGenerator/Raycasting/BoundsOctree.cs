...
public class BoundsOctree<T>
{
    ...
    public Bounds MaxBounds { get { return _rootNode.Bounds; } }
    ...
    private readonly Queue<Bounds> _lastBoundsCollisionChecks = new Queue<Bounds>();
    private readonly Queue<Ray> _lastRayCollisionChecks = new Queue<Ray>();
#endif
    ...
    {
        var removed = _rootNode.Remove(obj);
    ...
    {
        var removed = _rootNode.Remove(obj, objBounds);
    ...
    {
        var count = 0;
        foreach (var collisionCheck in _lastBoundsCollisionChecks)
        {
        ...
        foreach (var collisionCheck in _lastRayCollisionChecks)
        {
            Gizmos.color = new Color(1.0f, 1.0f - ((float)count / NumCollisionsToSave), 1.0f);
    ...
    private void Grow(Vector3 direction)
    {
        ...
        var half = _rootNode.BaseLength / 2;
        var newLength = _rootNode.BaseLength * 2;
        var newCenter = _rootNode.Center + new Vector3(xDirection * half, yDirection * half, zDirection * half);
